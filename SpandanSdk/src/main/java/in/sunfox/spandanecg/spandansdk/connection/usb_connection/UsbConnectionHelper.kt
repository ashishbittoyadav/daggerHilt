...
import `in`.sunfox.spandanecg.spandansdk.connection.usb_connection.ConstantHelper.Intents.ACTION_USB_PERMISSION
...
object UsbConnectionHelper {
    ...
    private lateinit var verificationHandler: Handler
    ...
    fun bind(usbConnectionContract: UsbConnectionContract, activity: Context) {
        ...
        SearchDeviceThread(usbManager).start()
    }
...
//        verificationHandler = Handler(Looper.getMainLooper())
//        verificationRunnable = Runnable{
//            if (!isDeviceVerified && isDeviceConnected) {
//                usbConnectionContract.onVerificationTimeout()
//            }
//        }
//        verificationHandler.postDelayed(verificationRunnable, 5000)
    }
...
//        Toast.makeText(,"Received",Toast.LENGTH_SHORT).show()
        ...
        if (finalData == "spdn" && !isDeviceVerified) {
            val handler = Handler(Looper.getMainLooper())
            handler.postDelayed({
                serialPort.write("d".toByteArray())
                Log.d("RawData.DATA", "Writing 'd'")
            }, 0)
...
//            usbConnectionContract.onDeviceVerified()
        ...
        else if(finalData.startsWith(ConstantHelper.B2B)){
            isDeviceVerified = true
            usbConnectionContract.onDeviceVerified()
        }
